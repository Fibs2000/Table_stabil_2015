...
ConnectFour::ConnectFour(NESController *controller, Adafruit_WS2801 *strip, RGBmatrixPanel *matrix):Game(controller, strip, matrix){
	newGame();
}
...
void ConnectFour::newGame(){
	...
	for(int i=0;i<9;i++){
		board[0][xOffset+i].setColor(0x009900);
		board[7][xOffset+i].setColor(0x222222);
		board[9][xOffset+i].setColor(0x009900);
	}
	for(int i=0;i<8;i++){
		board[1+i][xOffset].setColor(0x009900);
		board[1+i][xOffset+8].setColor(0x009900);
	}
	redrawBoard();
	updateDisplay();
}
